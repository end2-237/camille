...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Cas distinct de la déconnexion : WhatsApp répond, la session est vivante,
    // mais le workflow qui fabrique les réponses ne répond plus (arrêté, 404,
    // serveur tombé). Le client écrit et n'obtient rien, et jusqu'ici personne
    // ne l'apprenait — il a fallu lire les journaux du serveur pour le voir.
    //
    // On ne touche PAS au statut stocké : la session est bel et bien connectée,
    // et l'écrire en panne enverrait le vendeur rescanner un QR code inutile.
