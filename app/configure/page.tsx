// ─────────────────────────────────────────────────────────────────────────────
// app/configure/page.tsx — Camille by Buyticle
// Formulaire simplifié 3 étapes — même style immersif que le reste du site
// ─────────────────────────────────────────────────────────────────────────────
...
import { useCreateAgent } from "@/hooks/useLocalAgents";
...
const schema = z.object({
  ...
  // Step 3 — no extra fields, just confirm
});
      ...
      <Field label="Décrivez votre activité en quelques mots" error={errors.description?.message}>
        <div className="relative">
          ...
          <textarea
            ...
            rows={3}
            placeholder="Nous sommes une boutique de mode premium qui propose des pièces sélectionnées..."
    ...
    <div className="space-y-5">
          ...
          return (
            <div
              ...
              style={{
                ...
                background: i % 2 === 0 ? "var(--bg-muted)" : "var(--bg-base)",
              }}
            >
      ...
      >
        ...
        <p className="text-xs leading-relaxed" style={{ color: "var(--text-tertiary)" }}>
          Le system prompt sera généré automatiquement à partir de ces informations. Vous pourrez l'affiner dans le dashboard.
        </p>
      </div>
    </div>
  );
...
export default function ConfigurePage() {
  ...
  // Per-step field names to validate
  ...
  const onSubmit = handleSubmit(async (data) => {
    setLoading(true);
    try {
      // Build full AgentFormData with defaults for optional fields
      const full: AgentFormData = {
      ...
      const prompt = generateSystemPrompt(full, full.target_model);
      const agent  = save(full, prompt);
      toast.success(`Agent "${agent.identity.name}" créé !`, {
        description: "Prêt dans votre dashboard.",
      });
  ...
  return (
    <main className="relative min-h-dvh flex flex-col items-center justify-center py-24 overflow-hidden">
      ...
      {/* Inner wrapper — centré, padding horizontal propre sur tous écrans */}
      <div className="relative z-10 w-full max-w-lg mx-auto px-4 sm:px-6">
        ...
        >
              ...
              {[0, 1].map((i) => (
                <div key={i} className="flex-1 flex" style={{ justifyContent: i === 0 ? "flex-end" : "flex-start", paddingLeft: i === 1 ? 8 : 0, paddingRight: i === 0 ? 8 : 0 }}>
                  <div
                    ...
                    style={{
                      background: step > i + 1 ? "rgba(212,175,55,0.45)" : "var(--border-subtle)",
                      marginLeft: i === 0 ? "50%" : 0,
                      marginRight: i === 1 ? "50%" : 0,
                    }}
                  />
                </div>
              ))}
              ...
              return (
                <div key={s.id} className="relative flex flex-col items-center gap-2 flex-1">
                  <div
                    ...
                    style={{
                      background: done   ? "rgba(212,175,55,0.22)" : active ? "rgba(212,175,55,0.12)" : "var(--bg-muted)",
                      border: done || active ? "1px solid rgba(212,175,55,0.4)" : "1px solid var(--border-subtle)",
                      boxShadow: active ? "0 0 12px rgba(212,175,55,0.25)" : "none",
                    }}
                  >
                    {done
                      ? <Check className="w-3.5 h-3.5" style={{ color: "var(--color-gold)" }} />
                      : <Icon  className="w-3.5 h-3.5" style={{ color: active ? "var(--color-gold)" : "var(--text-disabled)" }} />
                    }
                  </div>
                  <span
          ...
          >
            {/* Gold hairline — inside relative card */}
            ...
            <div className="h-0.5 w-full" style={{ background: "var(--border-subtle)" }}>
              <motion.div
                ...
                className="h-full"
                style={{ background: "linear-gradient(90deg, #D4AF37, #EDD96A)" }}
              />
            </div>
